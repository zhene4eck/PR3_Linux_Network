...
- Адрес-сети - сеть, в которой мы находимся в общем.
- Адрес-хоста - наше устройство в этой сети.
...
Задача: поделить сеть: 192.168.1.0/24 на 4 подсети:

1. префикс /24 говорит нам о том, что 24 бит занмает адрес сети, а адреса хоста занимают (32-24) = 8 бит.
Следовательно всего наша сеть будет иметь 2^8 = 256 адресов хоста.

2. Нам нужно пределить число битов, которые мы заимствуем у хостов для 4 сетей. 
Также как и в 1 пункте: 4 = 2^2, следовательно 2 бита приходится на 4 посдести (их мы заимствуем у хостовой части).

3. Так как число хотсов меняется, у нас меняется маска: заимстствуя 2 бит из хоста в адресную часть мы получаем маску в префиксной записи (/24+2) = /26. В обычной записи маски это: 255.255.255.192.

3.1. Чтобы понять, как получили число 192, можно обратиться к двоичносму представлению:
- Маска /24 — 255.255.255.0 (11111111.11111111.11111111.00000000). Заимствуем 2 бита: 11111111.11111111.11111111.11000000.
- Октет IP — 8 бит, веса позиций: 128 (2^7), 64 (2^6), 32 (2^5), 16 (2^4), 8 (2^3), 4 (2^2), 2 (2^1), 1 (2^0):

      1 × 128 = 128 (7-я позиция слева).
...
- Итого: 128 + 64 = 192.
- Мы не сможем определить маску в виде числовой и двоичной записи для сети не имея префикной записи.
3.2. Из остатка получаем размер 1-й подсети из 4-х:
- (/32 - /26) = 6 битов, выделенных на хостовую часть
- В колличественном эквиваленте это (2^6) = 64
- Однако диапазоны задействуют 2 запасных бита (1 и последний (64) в диапазоне), поэтому получаем 62 хоста

4. Подсети, которые мы получили по диапазонам:
...
| 3 | 192.168.1.128 | 192.168.1.129–190 | 192.168.1.191 |
| 4 | 192.168.1.192 | 192.168.1.193–254 | 192.168.1.255 |
| число из запроса | без хостовой части (не обязательно, что оканчивается на 0) | возможные IP в адресах сети (конкретный IP) | широковещательный |

Broadcast — это специальный адрес в каждой подсети, используемый для широковещательной рассылки сообщений всем устройствам внутри подсети.

**Роль Broadcast в таблице подсетей**
В таблице подсетей broadcast указан как последний адрес блока (все хостовые биты = 1). Например, в 192.168.1.0/26 broadcast = 192.168.1.63.

- Служит для отправки пакетов всем хостам подсети одновременно (ARP-запросы, DHCP Discover).
- Не назначается реальным устройствам — резервируется для протоколов.
- Пакет на .63 получат все хосты 192.168.1.1–62.

### Определяем адрес сети для 192.167.38.54/13 путем побитового сложения

1. /13 - 13 бит на адрес сети. В двоичной записи этот префикс равен 11111111.11111000.00000000.00000000
2. Представляем IP в двоичном виде 192.167.38.54 = 11000000.10100111.00100110.00110110 (по октетам).
3. Побитово складываем (как столбиком, 1+1=1, 1+0=0, 0+0=0) = 11000000.10100000.00000000.00000000.
4. Переводим из двоичного кода в обычную запись (пример также выше), получим 192.160.0.0.

### Личные (частные / серые) IP-адреса
Для разделения личных (частных) и публичных сетей используется стандарт RFC 1918. Утилита ipcalc при анализе таких адресов обычно помечает их как Private Internet.

1. Личные (частные / серые) IP-адреса
Эти адреса предназначены для использования внутри домашних, офисных или корпоративных сетей. Они не маршрутизируются в интернете — это значит, что из интернета напрямую «достучаться» до устройства с таким IP нельзя.

| Класс | Диапазон адресов | Маска (CIDR) | Где используется |
| :---: | :---: | :---: | :---: |
| A | 10.0.0.0 — 10.255.255.255 | 10.0.0.0/8 | Крупные корпорации, провайдеры |
| B | 172.16.0.0 — 172.31.255.255 | 172.16.0.0/12 | 	Средние сети, учебные заведения |
| C | 192.168.0.0 — 192.168.255.255 | 192.168.0.0/16 | Домашние роутеры, малые офисы |
			
Мы имеем государственную подсеть (много белых IP, которые напрямую ведут нас в глобальный интернет), каждлый из этих белых IP выдается на город или район (условно), внтури них создается сеть ждя каждого дома, в каждом доме квартиры имеют роутеры (таже внутренняя отдельная сеть), что фактически создает огромную цепочку из отделных сетей, связанных, которые ведут нас последовательно к посдети страны, которая выводит нас в глобальный интернет мира. Поэтому в разных домах могут быть одинаковые IP (например, у всех роутер имеет 192.168.1.1), но они не конфликтуют, так как изолированы друг от друга (представляют свою сеть).

2. Публичные (белые) IP-адреса
Это все остальные адреса, которые не зарезервированы под специальные нужды. Они уникальны во всем мире.
Диапазоны: Все, что не входит в частные диапазоны выше (и не является спец. адресами вроде 127.0.0.1).
Примеры: 8.8.8.8 (Google), 140.82.121.4 (GitHub).
Как получить: Выдаются провайдером (ISP) или регистраторами (RIPE, ARIN).

4. Специальные адреса (не личные и не публичные)
127.0.0.0/8 — Loopback (тот самый localhost). Только для связи внутри одного устройства.
169.254.0.0/16 — APIPA. Эти адреса компьютер назначает себе сам, если не смог получить IP от роутера (сигнал о проблеме с сетью).
