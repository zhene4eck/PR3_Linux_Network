...
**Типы ICMP пакетов:**
echo-reply       (тип 0)  # Ответ на ping
echo-request     (тип 8)  # Запрос ping
destination-unreachable (тип 3) # Сеть/порт недоступен
time-exceeded    (тип 11) # TTL истек
...
Для ws1 (запрещающее → разрешающее):

- Сначала применяется правило -j DROP — оно блокирует все echo-reply пакеты
- Затем применяется правило -j ACCEPT — но оно никогда не сработает, потому что пакет уже был обработан первым правилом 
- Результат: машина НЕ будет отвечать на ping (заблокировано)

Для ws2 (разрешающее → запрещающее):

- Сначала применяется правило -j ACCEPT — оно разрешает все echo-reply пакеты
- Затем применяется правило -j DROP — но пакеты уже разрешены первым правилом
- Результат: машина будет отвечать на ping (разрешено)
