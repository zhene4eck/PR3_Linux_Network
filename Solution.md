...
На 1 машине прокладываем статичный маршрут к машине 2 от 1 (``ws-2 via enp0s10`` второй машины)
    ...
    ![alt text](Photo/пинг-1-2.png)
        ...
        Важно: 172.16.0.0/12 ``16`` мы получили как:
...
- для этого мы открывваем файл конфигурации у обеих машин командой ``sudo nano /etc/netplan/00-installer-config.yaml`` и вносим необходимые изменения:
