...
1. После подколчения нового адаптора получаем сеть epn0s10. Для машины 1 задаем временный IP из одной сети ``sudo ip addr add 10.0.0.1/24 dev enp0s10``, на машине 2 задаем временный IP ``sudo ip addr add 10.0.0.2/24 dev enp0s10``. Теперь у нас есть мост для обмена, фото используемой команды для машин 1 и 2 ниже:

    ![alt text](Photo/1-мост.png)

    ![alt text](Photo/2-мост.png)

2. (Вначале включаем у 2 и 3 адаптора у 2х машин неразборчивый режим на "разрешить все") На 1 машине прокладываем статичный маршрут к машине 2 от 1 (ws-2 via enp0s10 второй машины) ``sudo ip route add 172.16.0.0/12 via 10.0.0.2 dev enp0s10``

    ![alt text]()

3. На 2 машине рокладываем статичный маршрут к машине 1 (ws-1 via enp0s10 первой машины) ``sudo ip route add 1192.168.100.10/16 via 10.0.0.2 dev enp0s10``

    ![alt text]()

- На машине ws-1 я прокладываю путь через epn0s8 ``sudo ip route add 172.16.0.0/12 via 10.0.2.15/24 dev enp0s8`` где ``dev enp0s8`` это общая сеть (мост)

    ![alt text]()

- На машине ws-2 я прокладываю путь к ws-1 через epn0s8 ``sudo ip route add 192.168.0.0/16 via 10.0.2.15/24 dev enp0s8`` где ``dev enp0s8`` это общая сеть (мос
