...
Но пакет попадёт на первое правило и будет: DROP. До второго он никогда не дойдёт. То есть правило ACCEPT бесполезно.  

**8. Как на самом деле работает проверка**
...
- второе правило никогда не используется.

**10. Что делает весь скрипт**

| Итог: |
| :---: |
| 1️⃣ очищает firewall |
| 2️⃣ открывает: SSH (22), HTTP (80) |
| 3️⃣ запрещает ICMP echo reply (из-за порядка правил) |

**11. Как это бы написали правильно**
